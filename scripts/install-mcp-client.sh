...
SPRINTIQ_URL="https://www.sprintiq.ai"
...
#!/bin/bash
export SPRINTIQ_MCP_URL="${SPRINTIQ_MCP_URL:-https://www.sprintiq.ai/api/mcp/server}"
...
try {
  const result = execSync('curl -s -X GET "' + (process.env.SPRINTIQ_MCP_URL || 'https://www.sprintiq.ai/api/mcp/server') + '"', { encoding: 'utf8' });
  ...
  console.log('  2. The server URL:', process.env.SPRINTIQ_MCP_URL || 'https://www.sprintiq.ai/api/mcp/server');
...
1. **Register on SprintiQ**: Visit https://www.sprintiq.ai/signup
...
- `SPRINTIQ_MCP_URL`: Server URL (default: https://www.sprintiq.ai/api/mcp/server)
