...
import ExportSprintFolderToJiraModal from "@/components/workspace/modals/export-sprint-folder-to-jira-modal";
...
const hasJiraIntegration = (
  ...
  return space.projects
    ...
    .some((project) => project.type === "jira");
};
...
export default function SecondarySidebar({
  ...
  const [spaces, setSpaces] = useState(() => {
    // Filter out soft-deleted records from initial data
    const filteredInitialSpaces = initialSpaces
      .filter((space: any) => !space.deleted_at) // Filter out soft-deleted spaces
      .map((space: any) => ({
        ...space,
        projects: (space.projects || []).filter((p: any) => !p.deleted_at),
        sprint_folders: (space.sprint_folders || [])
          .map((sf: any) => ({
            ...sf,
            sprints: (sf.sprints || []).filter((s: any) => !s.deleted_at),
          }))
          .filter((sf: any) => !sf.deleted_at),
      }));
    return filteredInitialSpaces;
  });
  ...
  const [
    exportSprintFolderToJiraModalOpen,
    setExportSprintFolderToJiraModalOpen,
  ] = useState(false);
  const [selectedSprintFolderForExport, setSelectedSprintFolderForExport] =
    useState<{
      sprintFolder: SprintFolder;
      space: Space;
    } | null>(null);
  ...
  useEffect(() => {
    ...
    if (spaceMatch) {
      ...
      if (currentSpace) {
        console.log("Found current space for expansion:", currentSpace);
        ...
        console.log(
          "Current space not found in spaces:",
          currentSpaceId,
          "Available spaces:",
          spaces.map((s) => ({
            id: s.id,
  ...
  const refreshSpaces = useCallback(async () => {
    try {
      ...
      // Filter out soft-deleted projects, sprint folders, and sprints
      if (spacesData) {
        console.log("Raw spaces data:", spacesData);
        const filteredSpacesData = spacesData
          .filter((space: any) => !space.deleted_at) // Filter out soft-deleted spaces
          .map((space: any) => ({
            ...space,
            projects:
              space.projects?.filter((project: any) => !project.deleted_at) ||
              [],
            sprint_folders:
              space.sprint_folders
                ?.map((sf: any) => ({
                  ...sf,
                  sprints:
                    sf.sprints?.filter((sprint: any) => !sprint.deleted_at) ||
                    [],
                }))
                .filter((sf: any) => !sf.deleted_at) || [],
          }));
        console.log("Filtered spaces data:", filteredSpacesData);
        setSpaces(filteredSpacesData);
      } else {
        setSpaces([]);
      }
  ...
  useEffect(() => {
    ...
    const spacesSubscription = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log("Real-time spaces change detected:", payload);
  ...
  useEffect(() => {
    ...
    const handleTaskDeleted = (event: CustomEvent) => {
      console.log("Task deleted event received in sidebar:", event.detail);
      // Refresh task counts when a task is deleted from other components (debounced)
      debouncedFetchTaskCounts();
    };
    ...
    const handleSprintFolderCreatedEvent = (event: CustomEvent) => {
      console.log(
        "Sprint folder created event received in sidebar:",
        event.detail
      );
      // Refresh spaces when a sprint folder is created from other components
      refreshSpaces();
    };
    ...
    const handleSprintFolderFavorited = (event: CustomEvent) => {
      console.log(
        "Sprint folder favorited event received in sidebar:",
        event.detail
      );
      // Refresh sprint folder favorites in sidebar
      const savedFavorites = localStorage.getItem(
      ...
      if (savedFavorites) {
        try {
          const favorites = JSON.parse(savedFavorites);
          setSprintFolderFavorites(new Set(favorites));
        } catch (error) {
    ...
    window.addEventListener(
      "sprintFolderCreated",
      handleSprintFolderCreatedEvent as EventListener
    );
    window.addEventListener(
      "eventMarkedAsRead",
    ...
    window.addEventListener(
      "sprintFolderFavorited",
      handleSprintFolderFavorited as EventListener
    );
  ...
  const handleSpaceCreated = useCallback(
    async (
      ...
      const spaceWithDefaults = {
        ...newSpace,
        projects: (newSpace.projects || []).filter((p) => !p.deleted_at),
        sprint_folders: (newSpace.sprint_folders || []).filter(
          (sf) => !sf.deleted_at
        ),
      };
  ...
  const getStatusesForEntity = useCallback(
    (_entity: Project | Sprint, space: Space) => {
      if (!statuses.length) return [];

      return statuses.filter(
        (status) => status.type === "space" && status.space_id === space.id
      );
    },
    [statuses]
  );
  ...
  const handleDeleteSpace = useCallback(
    async (spaceId: string) => {
      ...
      try {
        ...
        const projectsInSpace = (space.projects || []).filter(
          (p: any) => !p.deleted_at
        );
        const sprintsInSpace = (space.sprint_folders || [])
          .filter((sf: any) => !sf.deleted_at)
          .flatMap((sf: any) =>
            (sf.sprints || []).filter((s: any) => !s.deleted_at)
          );
        ...
        if (projectsInSpace.length > 0) {
          const projectIds = projectsInSpace.map((p: any) => p.id);
        ...
        if (sprintsInSpace.length > 0) {
          const sprintIds = sprintsInSpace.map((s: any) => s.id);
        ...
        console.log("Space soft-deleted successfully:", space.id);

        // Update local state
        console.log("Updating local state, removing space:", spaceId);
        setSpaces((prev) => {
          const filtered = prev.filter((s) => s.id !== spaceId);
          console.log("Local state after filtering:", filtered);
          return filtered;
        });

        // Force refresh to ensure soft-deleted spaces are not included
        setTimeout(() => {
          console.log("Forcing refresh after space deletion");
          refreshSpaces();
        }, 100);
      ...
      try {
        ...
        for (const space of spaces) {
          for (const sprintFolder of space.sprint_folders) {
            const sprint = sprintFolder.sprints.find(
              (s: any) => s.id === sprintId
            );
            if (sprint) {
              foundSprint = sprint;
        ...
        setSpaces((prev) =>
          prev.map((space) => ({
            ...space,
            sprint_folders: space.sprint_folders.map((sprintFolder: any) => ({
              ...sprintFolder,
              sprints: sprintFolder.sprints.map((sprint: any) =>
                sprint.id === sprintId
    ...
    const filtered = spaces
      .map((space) => ({
        ...
        projects: space.projects.filter(
          (project: any) =>
            project.name.toLowerCase().includes(query) && !project.deleted_at
        ),
        // Keep sprint folders and sprints as they are (don't filter them)
        sprint_folders: space.sprint_folders.filter(
          (sprintFolder: any) => !sprintFolder.deleted_at
        ),
      }))
      .filter(
        (space) =>
          space.name.toLowerCase().includes(query) ||
          space.projects.filter((p: any) => !p.deleted_at).length > 0 ||
          space.sprint_folders.filter((sf: any) => !sf.deleted_at).length > 0
      );
      ...
      try {
        ...
        for (const space of spaces) {
          const project = space.projects.find(
            (p: any) => p.id === projectId && !p.deleted_at
          );
          if (project) {
        ...
        setSpaces((prev) =>
          prev.map((space) => ({
            ...space,
            projects: space.projects.filter(
              (project: any) => project.id !== projectId && !project.deleted_at
            ),
          }))
        );
  ...
  const handleDeleteProject = useCallback(
    async (projectId: string) => {
      ...
      try {
        ...
        if (tasksError) {
          console.error("Error soft deleting project tasks:", tasksError);
          throw tasksError;
        }
        ...
        const { error: statusesError } = await supabase
          .from("statuses")
          .update({ deleted_at: currentTimestamp })
          .eq("space_id", foundProject.space_id)
          .eq("type", "space");
        if (statusesError) {
          console.error("Error soft deleting project statuses:", statusesError);
          throw statusesError;
        }
        ...
        toast({
          ...
          description:
            "Project and all its related data have been soft deleted.",
        });
  ...
  const handleExportSprintFolderToJSON = useCallback(
    async (sprintFolder: SprintFolder) => {
      try {
        const response = await fetch(
          `/api/workspace/${params.workspaceId}/export/sprint-folder/json`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ sprintFolderId: sprintFolder.id }),
          }
        );
        ...
        if (result.success) {
          ...
          toast({
            title: "Export successful",
            description: `Exported sprint folder with ${result.data.summary.totalSprints} sprints and ${result.data.summary.totalTasks} tasks to JSON`,
          });
        } else {
          throw new Error(result.error);
        }
        ...
        toast({
          title: "Export failed",
          description:
            error.message || "Failed to export sprint folder to JSON",
          variant: "destructive",
        });
      }
    },
  ...
  const handleExportSprintFolderToCSV = useCallback(
    async (sprintFolder: SprintFolder) => {
      try {
        const response = await fetch(
          `/api/workspace/${params.workspaceId}/export/sprint-folder/csv`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ sprintFolderId: sprintFolder.id }),
        ...
        if (result.success) {
          ...
          toast({
            title: "Export successful",
            description: `Exported sprint folder to CSV`,
          });
        } else {
          throw new Error(result.error);
        }
        ...
        toast({
          title: "Export failed",
          description: error.message || "Failed to export sprint folder to CSV",
          variant: "destructive",
        });
      }
    },
  ...
  return (
    <TooltipProvider>
      <div className="w-64 workspace-secondary-sidebar-bg border-r workspace-border flex flex-col">
                    ...
                    {favorites.size > 0 && (
                      <div>
                        {spaces
                          ...
                          .map((space) => (
                            <div
                              key={`fav-space-${space.id}`}
                              className="group"
                            >
                              <div
                                className="flex items-center px-2 text-sm rounded-lg workspace-sidebar-text cursor-pointer"
                                onClick={() => toggleSpace(space.space_id)}
                              >
                                <div
                                  className={`w-3 h-3 rounded-sm mr-2 flex-shrink-0 flex items-center justify-center group-hover:scale-110 transition-transform duration-200 ${getIconColor(
                                    space.icon
                                  )}`}
                                >
                            ...
                            .map((project: any) => (
                              ...
                              >
                                <Link
                                  ...
                                  className="block"
                                >
                                  <div className="flex items-center px-2 py-1 text-sm rounded-lg workspace-sidebar-text cursor-pointer">
                                    <Hash className="w-3 h-3 mr-2 flex-shrink-0 group-hover:scale-110 transition-transform duration-200" />
                                ...
                                .map((sprint: any) => (
                                  ...
                                  >
                                    <Link
                                      ...
                                      className="block"
                                    >
                                      <div className="flex items-center px-2 text-sm rounded-lg workspace-sidebar-text cursor-pointer">
                                        <CirclePlay className="w-3 h-3 mr-2 flex-shrink-0 group-hover:scale-110 transition-transform duration-200" />
                                        <div className="flex-1 min-w-0">
                                          <span className="text-xs font-medium workspace-sidebar-text truncate block">
                                            {sprint.name}
                                          </span>
                        ...
                        {spaces.map((space) =>
                          space.sprint_folders
                            .filter(
                              (sprintFolder: any) =>
                                sprintFolderFavorites.has(sprintFolder.id) &&
                                !sprintFolder.deleted_at
                            )
                            .map((sprintFolder: any) => (
                              <div
                                key={`fav-sprint-folder-${sprintFolder.id}`}
                                className="group"
                              >
                                <Link
                                  href={`/${workspaceId}/space/${space.space_id}/sf/${sprintFolder.sprint_folder_id}`}
                                  className="block"
                                >
                                  <div className="flex items-center px-2 text-sm rounded-lg workspace-sidebar-text cursor-pointer">
                                    <FolderKanban className="w-3 h-3 mr-2 flex-shrink-0 group-hover:scale-110 transition-transform duration-200" />
                                    <div className="flex-1 min-w-0">
                                      <span className="text-xs font-medium workspace-sidebar-text truncate block">
                                        {sprintFolder.name}
                                      </span>
                                          ...
                                          <DropdownMenuPortal>
                                            <DropdownMenuSubContent>
                                              <DropdownMenuItem
                                                className="text-xs hover:workspace-hover cursor-pointer"
                                                onClick={() => {
                                                  setSelectedSprintFolderForExport(
                                                    {
                                                      sprintFolder,
                                                      space,
                                                    }
                                              ...
                                              <DropdownMenuItem
                                                className="text-xs hover:workspace-hover cursor-pointer"
                                                onClick={() =>
                                                  handleExportSprintFolderToJSON(
                                                    sprintFolder
                                                  )
                                                }
                                              >
        ...
        <ExportSprintFolderToJiraModal
          open={exportSprintFolderToJiraModalOpen}
          onOpenChange={setExportSprintFolderToJiraModalOpen}
          onSuccess={() => {
            // Refresh data after successful export
            refreshSpaces();
            fetchTaskCounts();
          ...
          workspace={workspace}
          spaces={spaces}
          selectedSprintFolder={selectedSprintFolderForExport?.sprintFolder}
          selectedSpace={selectedSprintFolderForExport?.space}
        />
