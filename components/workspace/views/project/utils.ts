...
export const filterTasks = (
  ...
  return tasks.filter((task) => {
    ...
    if (filters.showUnassignedOnly) {
      ...
      if (task.assignee_id || task.assigned_member_id) {
        return false;
      }
    } else if (filters.assigned.length > 0) {
      // Show only tasks assigned to specific users
      const isAssignedToSelectedUser = filters.assigned.some((userId) => {
        // Check if this is a team member assignment (starts with "team-")
        if (userId.startsWith("team-")) {
          const teamMemberId = userId.replace("team-", "");
          return task.assigned_member_id === teamMemberId;
        } else {
          // Check for workspace member assignment
