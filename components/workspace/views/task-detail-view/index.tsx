...
export default function TaskDetailView({
  ...
  const addAssignee = async (memberId: string) => {
    try {
      ...
      const isTeamMember = memberId && memberId.startsWith("team-");
      const actualMemberId = isTeamMember
        ? memberId.replace("team-", "")
        : memberId;
      ...
      if (isTeamMember) {
        member = teamMembers.find((m) => m.id === actualMemberId);
        memberName = member?.profile?.full_name || member?.name || "Unknown";
      } else {
        member = workspaceMembers.find((m) => m.id === actualMemberId);
      ...
      if (isTeamMember) {
        updateData.assigned_member_id = actualMemberId;
        updateData.assignee_id = null; // Clear profile assignee
      } else {
        updateData.assignee_id = actualMemberId;
      ...
      // Update local state
      const assigneeData = isTeamMember
        ? { id: memberId, name: memberName, type: "team" }
        : { id: memberId, full_name: memberName, type: "profile" };

      setTaskAssignees([assigneeData]);

      const updatedTask = {
        ...task,
        assignee_id: isTeamMember ? null : actualMemberId,
        assigned_member_id: isTeamMember ? actualMemberId : null,
        assignee: isTeamMember ? null : member,
        assigned_member: isTeamMember ? member : null,
      };
      setTask(updatedTask);
      setEditedTask(updatedTask);
      ...
      if (user) {
        ...
        const newAssigneeName = memberName;
  ...
  const removeAssignee = async (memberId: string) => {
    try {
      ...
      const updatedTask = {
        ...task,
        assignee_id: isTeamMember ? task.assignee_id : null,
        assigned_member_id: isTeamMember ? null : task.assigned_member_id,
        assignee: isTeamMember ? task.assignee : null,
        assigned_member: isTeamMember ? null : task.assigned_member,
      };
      setTask(updatedTask);
      setEditedTask(updatedTask);
