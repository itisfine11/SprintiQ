...
export const useTaskActions = (
  ...
  const handleUpdateTimeEstimate = async (timeEstimate: number | null) => {
    setLoading(true);
    try {
      const oldTimeEstimate = initialTask.estimated_time;
  ...
  const handleUpdateStoryPoints = async (storyPoints: number | null) => {
    setLoading(true);
    try {
      const oldStoryPoints = initialTask.story_points;
      ...
      if (user) {
        const oldValue = oldStoryPoints || "None";
        const newValue = storyPoints || "None";
        ...
        await createTaskActivity({
          ...
          spaceId: space.id,
          projectId: project?.id,
          description: `changed story points from ${oldValue} to ${newValue}`,
          metadata: {
            oldValue: oldStoryPoints,
            newValue: storyPoints,
          },
        });
