...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  // Debug logging
  console.log("TaskCard teamMembers:", teamMembers?.length || 0, teamMembers);
  const {
    attributes,
    listeners,
    setNodeRef,
                ...
                <div className="flex items-center justify-center">
                  {task.assignee ? (
                    ...
                    <Avatar className="h-6 w-6">
                      <AvatarImage
                        src={
                          task.assigned_member?.profile?.avatar_url ?? undefined
                        }
                        alt={
                      ...
                      <AvatarFallback className="text-xs workspace-component-bg workspace-component-active-color">
                        {getAvatarInitials(
                          task.assigned_member?.profile?.full_name ||
                            task.assigned_member?.name,
                          task.assigned_member?.profile?.email ||
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    ...
                    >
                      ...
                      {!task.assignee_id && !task.assigned_member_id && (
                        ...
                        {teamMembers.map((member: any) => {
                          ...
                          return (
                            <CommandItem
                              key={`team-${member.id}`}
                              onSelect={() =>
                                onAssignTask(task.id, `team-${member.id}`)
                              }
                              className="flex items-center justify-between cursor-pointer"
                              onClick={(e) => e.stopPropagation()}
                            >
