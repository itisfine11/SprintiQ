...
// Function to check if a path is a public insight route
const isPublicInsightRoute = (pathname: string) => {
  // Check if it's the main insights page
  if (pathname === "/insights") return true;

  // Check if it's an insight detail page (pattern: /insights/i1234567890)
  const insightDetailPattern = /^\/insights\/i\d{10}$/;
  return insightDetailPattern.test(pathname);
};
...
export const config = {
  matcher: [
    "/((?!_next/static|_next/image|favicon.ico|.*\\.png$|sitemap\\.xml|sitemap-index\\.xml|robots\\.txt).*)",
  ],
};
