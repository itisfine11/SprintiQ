...
export async function analyzeTAWOSDataset(issues: TAWOSIssue[]): Promise<{
  ...
  try {
    ...
    const prompt = `
      Analyze this Jira dataset to extract patterns and insights for story generation.
    ...
    try {
      ...
      console.log("Failed to parse analysis:", parseError);
      return {
        patterns: {
          storyPointPatterns: {},
          priorityPatterns: {},
    ...
    console.log("Error analyzing dataset:", error);
    return {
      patterns: {
        storyPointPatterns: {},
        priorityPatterns: {},
