...
export class JiraSyncService {
  ...
  private async updateStatusesFromJira(jiraStatuses: any[]): Promise<number> {
    ...
    for (const jiraStatus of jiraStatuses) {
      const { data: existingStatus } = await this.supabase
        .from("statuses")
        .select("*")
        .eq("workspace_id", this.project.workspace_id)
        .eq("space_id", this.project.space_id)
        .eq("type", "space")
  ...
  private async pushTasksToJira(): Promise<number> {
    ...
    const { data: newTasks, error: newTasksError } = await this.supabase
      ...
      .select("*")
      .eq("project_id", this.project.id)
      .in("type", ["jira", "default", "ai-generated"])
  ...
  private mapJiraPriority(priority?: string): string {
    return mapJiraPriorityToLocal(priority);
  }

  private mapPriorityToJira(priority: string): string {
    return mapLocalPriorityToJira(priority);
  }
  ...
  private convertDescriptionForJira(description: string | null): string {
    ...
    if (typeof description === "string") {
      ...
      // Check if it's the generated story format and convert to Jira format
      if (
        description.includes("## User Story") ||
        description.includes("## Acceptance Criteria")
      ) {
        return this.convertGeneratedStoryToJiraFormat(description);
      }
   ...
   */
  private convertGeneratedStoryToJiraFormat(storyContent: string): string {
  ...
  testHtmlToJiraConversion(): {
    ...
    example3: { generatedStory: string; jira: string };
  } {
...
This task was generated using AI story generation and may require review and refinement.`;
    ...
    const example3GeneratedStory = `## User Story
...
`;
    ...
    return {
      ...
      example3: {
        generatedStory: example3GeneratedStory,
        jira: this.convertGeneratedStoryToJiraFormat(example3GeneratedStory),
      },
    };
  }
   ...
   */
  private extractStoryPoints(jiraIssue: any): number | null {
    ...
    for (const fieldName of storyPointsFields) {
      const fieldValue = jiraIssue.fields[fieldName];
  ...
  /**
   * Test method to verify story points extraction
   */
  testStoryPointsExtraction(): {
    testCases: Array<{
      description: string;
      jiraIssue: any;
    ...
    const testCases = [
      {
        description: "Story points as number in customfield_10016",
        jiraIssue: {
          fields: {
            customfield_10016: 5,
          },
      ...
      {
        description: "Story points as string in customfield_10016",
        jiraIssue: {
          fields: {
            customfield_10016: "8",
          },
      ...
      {
        description: "Story points as object with value property",
        jiraIssue: {
          fields: {
            customfield_10016: { value: 13 },
          },
        },
        expectedResult: 13,
      },
      {
        description: "Story points in customfield_10036",
        jiraIssue: {
          fields: {
            customfield_10036: 3,
          },
      ...
      {
        description: "No story points field",
        jiraIssue: {
          fields: {
            summary: "Test issue",
          },
      ...
      {
        description: "Invalid story points value",
        jiraIssue: {
          fields: {
    ...
    const results = testCases.map((testCase) => {
      const result = this.extractStoryPoints(testCase.jiraIssue);
