...
async function getEnhancedTeamAssignment(
  ...
  console.log(
    `👥 Team members: ${teamMembers
      .map(
        (m) =>
          `${m.name} (${m.role}, ${m.level}, skills: [${m.skills.join(", ")}])`
      )
      .join(", ")}`
  );
  ...
  const memberScores = teamMembers.map((member) => {
    ...
    console.log(
      `📊 ${member.name} (${member.level}): skill=${skillMatch.toFixed(
        ...
        1
      )}/${maxWorkload.toFixed(1)}h), distribution=${distributionScore.toFixed(
        2
      )} (${currentAssignmentCount}/${maxStoriesPerMember} stories), total=${totalScore.toFixed(
        2
      )}`
    );
  ...
  if (availableMembers.length === 0) {
    console.warn(
      `⚠️ All team members have reached their assignment limit (${maxStoriesPerMember} stories)`
    );
  ...
  console.log(
    `   - Distribution score: ${(bestMatch.distributionScore * 100).toFixed(
      0
    )}% (${bestMatch.currentAssignmentCount}/${maxStoriesPerMember} stories)`
  );
...
function calculateSkillMatch(
  ...
  const finalScore = Math.min(totalMatches / maxPossibleMatches, 1.0);

  console.log(
    `🔍 Skill match for ${
      member.name
    }: direct=${directMatches}, partial=${partialMatches}, role=${roleSkillMatch.toFixed(
      2
...
function getLocalTeamAssignment(
  ...
  if (availableMembers.length === 0) {
    ...
    // If all members are at limit, return the best match anyway
...
export async function testImprovedTeamAssignment(): Promise<void> {
  console.log("🧪 Testing Improved Team Assignment Distribution");
  ...
  console.log(
    `📊 Testing with ${sampleTeamMembers.length} team members and ${testStories.length} stories`
  );
  console.log("Team Members:");
  sampleTeamMembers.forEach((member) => {
    console.log(
      `  - ${member.name} (${member.level}): ${member.skills.join(", ")}`
    );
  });
  console.log();
  ...
  for (const story of testStories) {
    console.log(
      `\n--- Testing: ${story.title} (${story.priority} priority, ${story.complexity} complexity) ---`
    );
    ...
    try {
      ...
      if (result.assignedMember) {
        ...
        console.log(`   Reason: ${result.reason}`);
        console.log(`   Confidence: ${(result.confidence * 100).toFixed(0)}%`);
        console.log(`   Skill Match: ${(result.skillMatch * 100).toFixed(0)}%`);
        console.log(`   Current assignments: ${currentCount + 1}/3`);
      } else {
        console.log(`❌ No assignment possible`);
      }
    } catch (error) {
  ...
  sampleTeamMembers.forEach((member) => {
    ...
    console.log(`\n${member.name} (${member.level}): ${count} stories`);
    memberAssignments.forEach((assignment) => {
      console.log(`  - ${assignment.story} (${assignment.priority} priority)`);
    });
  });

  // Show priority distribution analysis
  console.log("\n" + "=".repeat(60));
  console.log("🎯 PRIORITY DISTRIBUTION ANALYSIS:");
  console.log("=".repeat(60));
  ...
  Object.entries(priorityDistribution).forEach(([priority, assignments]) => {
    console.log(`\n${priority} Priority Stories:`);
    assignments.forEach((assignment) => {
      console.log(
        `  - ${assignment.story} → ${assignment.member} (${assignment.level})`
      );
    });
  });

  console.log("\n✅ Improved Team Assignment Test Complete");
}
