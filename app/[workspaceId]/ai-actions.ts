  ...
  try {
    ...
    - ${persona.name} (${persona.role || "User"}):
      * Tech Savviness: ${persona.tech_savviness || 3}/5 (${
          persona.tech_savviness === 1
            ? "Beginner"
            : persona.tech_savviness === 2
            ? "Novice"
            : persona.tech_savviness === 3
            ? "Intermediate"
            : persona.tech_savviness === 4
            ? "Advanced"
            : "Expert"
        })
      * Domain: ${persona.domain || "General"}
      * Usage Frequency: ${persona.usage_frequency || "Weekly"}
    ...
    ADJUST STORY COMPLEXITY AND LANGUAGE BASED ON PERSONA TECH-SAVVINESS:
    - Tech Level 1-2 (Beginner/Novice): Use simple language, avoid technical jargon, focus on basic functionality
    - Tech Level 3 (Intermediate): Balance technical and business language, moderate complexity
    - Tech Level 4-5 (Advanced/Expert): Use technical terminology, include advanced features and integrations
    `
        : "";
    ...
    - Dependencies (${priorityWeights.dependencies}%): Reliance on other components or systems
      ...
      9. ADJUST LANGUAGE AND COMPLEXITY TO MATCH PERSONA TECH-SAVVINESS
      10. INCLUDE ANTI-PATTERN WARNINGS FOR POTENTIAL FAILURE POINTS
      11. ENSURE STAKEHOLDER ALIGNMENT AND CLEAR COMMUNICATION
      12. AVOID FBI Sentinel anti-patterns:
      ...
      - Persona-appropriate language and complexity
    ...
    const message = await anthropic.messages.create({
      ...
      max_tokens: Math.max(3000, numberOfStories * 500), // Increase tokens for more stories
      temperature: 0.7,
      messages: [
        {
          role: "user",
      ...
      if (jsonMatch) {
        ...
        if (!parsedSuccessfully) {
          ...
          if (extractedStories.length > 0) {
            ...
            // If we extracted fewer than requested, fill with fallback stories
      ...
      const enhancedStories = await Promise.all(
        stories.map(async (story, index) => {
          ...
          if (teamMembers.length > 0) {
            try {
              // Track current assignments for distribution
              const currentAssignments = new Map<string, number>();

              // Count current assignments from previous stories in this batch
              for (let i = 0; i < index; i++) {
                const previousStory = stories[i];
                if (previousStory.assignedTeamMember?.id) {
                  const currentCount =
                    currentAssignments.get(
                      previousStory.assignedTeamMember.id
                    ) || 0;
              ...
              const assignment = await getOptimalTeamAssignment(
                ...
                teamMembers,
                {
                  ...
                  maxStoriesPerMember: Math.max(
                    2,
                    Math.ceil(stories.length / teamMembers.length)
                  ), // Distribute stories evenly
                  currentAssignments,
                }
              );
              ...
              if (assignment.assignedMember) {
                ...
                // Check if skill match is below 50% and add warning
                const skillMatchPercentage = assignment.skillMatch * 100;
                if (skillMatchPercentage < 50) {
                  enhancedStory.antiPatternWarnings =
                    enhancedStory.antiPatternWarnings || [];
                  enhancedStory.antiPatternWarnings.push(
                    `LOW SKILL MATCH: ${
                      assignment.assignedMember.name
                    } has only ${skillMatchPercentage.toFixed(
                      0
                    )}% skill match for this story. Consider working with ${
                      assignment.assignedMember.name
                    } to provide additional training or pair programming support.`
                  );
                }
                ...
                ).toFixed(0)}%, Skill Match: ${skillMatchPercentage.toFixed(
                  0
                )}%)`;
              } else {
              }
              ...
              // Fallback to simple assignment based on skills with distribution
              const currentAssignments = new Map<string, number>();
              ...
              // Find best match considering distribution
              const maxStoriesPerMember = Math.max(
                2,
                Math.ceil(stories.length / teamMembers.length)
              );
              const availableMembers = teamMembers.filter((member) => {
                const currentCount = currentAssignments.get(member.id) || 0;
                return currentCount < maxStoriesPerMember;
              });

              const bestMatch =
                availableMembers.find((member) =>
                ...
                ) || availableMembers[0]; // Fallback to first available member

              if (bestMatch) {
                enhancedStory.assignedTeamMember = bestMatch;

                // Calculate simple skill match for fallback
                const matchingSkills = bestMatch.skills.filter((skill) =>
                  (story.tags || []).some(
                    (tag) =>
                      skill.toLowerCase().includes(tag.toLowerCase()) ||
                      tag.toLowerCase().includes(skill.toLowerCase())
                ...
                const skillMatchPercentage =
                  story.tags && story.tags.length > 0
                    ? (matchingSkills.length / story.tags.length) * 100
                    : 50; // Default to 50% if no tags
                ...
                enhancedStory.description = `${
                  ...
                  bestMatch.name
                } (skill match fallback, Skill Match: ${skillMatchPercentage.toFixed(
                  0
                )}%)`;
              } else {
              }
...
export async function saveUserStory(
  ...
  try {
    console.log("=== saveUserStory START ===");
    ...
    console.log("Looking up workspace UUID for workspaceId:", workspaceId);
    ...
    console.log("Workspace UUID found:", workspace.id);

    // Get the first space-level status for the project's space
    console.log("Looking up project info for projectId:", projectId);
    ...
    console.log("Looking up statuses for space_id:", projectInfo.space_id);
    const { data: statuses, error: statusesError } = await supabase
      .from("statuses")
      .select("id, name, type, space_id, project_id, sprint_id")
      .eq("workspace_id", workspace.id) // Use workspace UUID, not workspace_id string
      .eq("space_id", projectInfo.space_id)
      .is("project_id", null)
      .is("sprint_id", null)
    ...
    if (!statuses || statuses.length === 0) {
      console.error("No statuses found for space_id:", projectInfo.space_id);
      return { success: false, error: "No status found for this project" };
    }

    const statusId = statuses[0].id;
    console.log("Using status:", { id: statusId, name: statuses[0].name });
...
- Priority: ${story.priority || "Not specified"}
    ...
    console.log("Inserting task with status_id:", statusId);
    ...
    const { data: insertedTask, error } = await supabase.from("tasks").insert({
      ...
      workspace_id: workspace.id, // Use workspace UUID, not workspace_id string
      created_by: user.id,
      embedding: embedding,
      assigned_member_id: story.assignedTeamMember?.id || null,
      assignee_id: null, // Don't set assignee_id for team members
    });
    ...
    console.log("=== saveUserStory SUCCESS ===");
...
export async function createTaskWithAI(
  ...
  try {
    console.log("=== createTaskWithAI START ===");
    console.log("Parameters:", { projectId, taskName, workspaceId });
    ...
    // First get the workspace UUID from workspace_id
    console.log("Looking up workspace for workspaceId:", workspaceId);
    const { data: workspace, error: workspaceError } = await supabase
      .from("workspaces")
      .select("id")
      .eq("workspace_id", workspaceId)
    ...
    console.log("Looking up project for projectId:", projectId);
    ...
    console.log("Creating task with status_id:", statusId);
    // Create the task
    const { data: newTask, error: taskError } = await supabase
      ...
      .insert({
        ...
        assigned_member_id: null, // No team member assigned for manually created tasks
      })
    ...
    try {
      console.log("Creating event for task creation...");
    ...
    console.log("=== createTaskWithAI SUCCESS ===");
    return {
      success: true,
...
export async function createSpaceAndProject(
  ...
  try {
    console.log("=== createSpaceAndProject START ===");
    console.log("Parameters:", {
      workspaceId,
      spaceName,
      projectName,
    ...
    if (existingSpaceId && existingSpaceId.length > 0) {
      console.log("Using existing space:", existingSpaceId);
      ...
      console.log("Existing space found:", { spaceId, spaceUuid });
    }
    // Create new space
      ...
      if (memberError) {
        console.error("Failed to add user as space member:", memberError);
      } else {
        console.log("User added as space member");
      }
    ...
    // Create statuses at space-level with colors
    console.log("Creating statuses:", statusNames);
    const statusInserts = statusNames.map((statusName, index) => ({
      ...
      position: index,
      type: "space",
      project_id: null, // Explicitly null for space-level statuses
      space_id: spaceUuid,
      sprint_id: null, // Explicitly null for space-level statuses
      workspace_id: workspace.id,
    }));

    console.log("Status inserts:", statusInserts);
    ...
    console.log("Statuses created successfully:", createdStatuses);

    console.log("=== createSpaceAndProject SUCCESS ===");
    return {
      success: true,
      spaceId: spaceId,
      projectId: newProject.project_id,
...
export async function saveUserStoryToDestination(
  ...
  try {
    console.log("=== saveUserStoryToDestination START ===");
    console.log("Parameters:", {
      workspaceId,
      destination,
      storyTitle: story.title,
    ...
    if (destination.sprintId) {
      ...
      console.log("Project mode - spaceId:", spaceId, "projectId:", projectId);
    }
    ...
    if (destination.sprintId) {
      ...
      if (destination.statusId) {
        ...
        console.log("Using provided statusId for sprint:", statusId);
      } else {
      ...
      // For project mode, find status by space_id (unified status system)
      console.log("Looking up space-level statuses for space_id:", spaceId);
      ...
      console.log("Project statuses found:", projectStatuses);

      if (projectStatuses && projectStatuses.length > 0) {
        statusId = projectStatuses[0].id;
        console.log("Using status:", {
          id: statusId,
          name: projectStatuses[0].name,
        });
      } else {
        console.error("No statuses found for space_id:", spaceId);
        return {
          success: false,
          error:
            "No status found for this space. Please create a status first.",
        };
      }
    }
    ...
    if (
      ...
      console.log("Saving new team member:", story.assignedTeamMember.name);
      ...
      if (memberResult.success && memberResult.memberId) {
        ...
        console.log("Team member saved with ID:", assignedMemberId);
      }
    }

    // If we have an assigned team member, save their team member ID separately
    let assignedMemberIdForTask: string | undefined;
    let assigneeId: string | undefined;

    if (story.assignedTeamMember) {
      if (assignedMemberId) {
        // Use the saved team member ID for assigned_member_id
        assignedMemberIdForTask = assignedMemberId;
        console.log("Using saved team member ID:", assignedMemberIdForTask);
        // Don't set assignee_id for team members - it should only be used for profile users
      } else if (!story.assignedTeamMember.id.startsWith("member-")) {
        // This is an existing team member, use their ID for assigned_member_id
        assignedMemberIdForTask = story.assignedTeamMember.id;
        console.log("Using existing team member ID:", assignedMemberIdForTask);
        // Don't set assignee_id for team members - it should only be used for profile users
      }
    }
    ...
    console.log("Generated story tags:", storyTags);
    ...
    console.log(
      "Inserting task with status_id:",
      destination.statusId || statusId
    );
    ...
    const { data: insertedTask, error: taskError } = await supabase
      ...
      .insert({
        ...
        assigned_member_id: assignedMemberIdForTask || null,
        workspace_id: workspace.id, // Use workspace UUID, not workspace_id string
        created_by: user.id,
        type: "ai-generated",
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("taskCreated", {
          detail: {
            task: insertedTask,
            project: destination.sprintId ? null : { id: projectId },
            sprint: destination.sprintId ? { id: destination.sprintId } : null,
          },
        })
      );
    }
    ...
    if (story.parentTaskId) {
      console.log("Setting parent task relationship:", story.parentTaskId);
      ...
      if (parentTask) {
        ...
        if (updateError) {
          ...
          console.log("Parent task relationship set successfully");
        }
      }
    }
    ...
    if (story.childTaskIds && story.childTaskIds.length > 0) {
      console.log("Setting child task relationships:", story.childTaskIds);
      const { error: updateError } = await supabase
        .from("tasks")
      ...
      if (updateError) {
        ...
        console.log("Child task relationships set successfully");
      }
    }

    // Create tags for the task
    console.log("Creating tags for task:", storyTags);
    for (const tagName of storyTags) {
      ...
      if (existingTag) {
        ...
        console.log("Using existing tag:", tagName, "ID:", tagId);
      } else {
        // Create new tag
      ...
      if (tagId) {
        ...
        console.log("Linked tag to task:", tagName);
      }
    }

    // Revalidate the project page to show the new task
    const revalidatePathString = `/${workspaceId}/space/${spaceId}/project/${projectId}`;
    console.log("Revalidating path:", revalidatePathString);
    revalidatePath(revalidatePathString);

    console.log("=== saveUserStoryToDestination SUCCESS ===");
...
export async function createSprints({
  ...
  try {
    console.log("=== createSprints START ===");
    console.log("Parameters:", {
    ...
    console.log("Sprints created successfully:", newSprint.length);

    // Check if a space-level status already exists
    console.log("Checking for existing space-level statuses...");
    ...
    if (existingStatuses && existingStatuses.length > 0) {
      console.log("Using existing space-level statuses:", existingStatuses);
      createdStatuses = existingStatuses;
    } else {
      // Create only one space-level status for all sprints (unified status system)
      console.log("Creating new space-level status...");
      const statusInsert = {
        name: "Backlog",
        color: "gray",
        position: 0,
      ...
      createdStatuses = newStatuses;
    }

    console.log("Status created successfully:", createdStatuses);
    console.log("=== createSprints SUCCESS ===");
