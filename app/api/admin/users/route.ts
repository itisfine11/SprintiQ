...
export async function GET(req: NextRequest) {
  ...
  if (validSortFields.includes(sort) && validOrderDirections.includes(order)) {
...
export async function PATCH(req: NextRequest) {
  ...
  if (typeof allowed === "boolean") {
    ...
    // If user is being granted access (allowed = true), send approval email
    if (allowed === true) {
      try {
        ...
        if (userData && userData.name && userData.email) {
          await sendApprovalEmail(id, userData.name, userData.email);
        }
      } catch (emailError) {
