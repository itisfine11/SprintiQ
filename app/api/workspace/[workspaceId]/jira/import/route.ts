...
export async function POST(
  ...
  try {
    ...
    for (const project of selectedJiraProjects) {
      try {
        console.log(`Processing project: ${project.key}`);

        // First, try to get the sprint field name for this project
        const sprintFieldName = await jiraAPI.getSprintFieldName(project.key);
        console.log(`Sprint field name for ${project.key}:`, sprintFieldName);
        ...
        console.log(`Found ${issues.length} issues for project ${project.key}`);

        // Debug: Check first few issues for sprint information
        for (let i = 0; i < Math.min(3, issues.length); i++) {
          const issue = issues[i];
          console.log(`Issue ${issue.key} fields:`, Object.keys(issue.fields));
          const sprintInfo = jiraAPI.extractSprintFromIssue(issue);
          console.log(`Sprint info for ${issue.key}:`, sprintInfo);
        }
    ...
    // Separate issues by sprint and non-sprint
    const sprintIssues: any[] = [];
    const nonSprintIssues: any[] = [];
    ...
    console.log(
      `Sprint issues: ${sprintIssues.length}, Non-sprint issues: ${nonSprintIssues.length}`
    );
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        sprintFolders: result.sprintFolders.length,
        sprints: result.sprints.length,
        sprintIssues: sprintIssues.length,
        nonSprintIssues: nonSprintIssues.length,
      },
    });
  } catch (error: any) {
